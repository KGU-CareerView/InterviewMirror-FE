  ...
  <div class="bg-gray-50 h-screen flex flex-col font-sans overflow-hidden">
    ...
    <main class="flex-1 w-full flex p-6 gap-6 h-full relative">
      ...
      <section class="w-[70%] bg-black rounded-3xl shadow-lg relative overflow-hidden flex flex-col items-center justify-center border-4 border-gray-800">
        ...
        <div v-if="!isCameraReady" class="absolute inset-0 bg-gray-950 text-white flex flex-col items-center justify-center px-8 text-center">
          <div class="w-16 h-16 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center mb-5">
            <svg class="w-8 h-8 text-brandLight" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
          </div>
    ...
    <div v-if="isCameraPermissionModalOpen && !isModalOpen" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-40 px-4">
      <div class="bg-white rounded-[24px] shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100">
          <button @click="continueWithoutCamera" class="px-5 py-3 rounded-xl font-bold text-gray-500 bg-white border border-gray-200 hover:bg-gray-50 hover:text-gray-700 transition-colors text-sm">
            카메라 없이 진행
          </button>
          <button
...
const cameraPermissionMessage = ref('실시간 표정 분석을 사용하려면 카메라 권한이 필요합니다. 카메라가 없거나 권한을 허용하지 않아도 질문 답변은 그대로 진행할 수 있습니다.')
...
const initCameraStream = async () => {
  if (!navigator.mediaDevices?.getUserMedia) {
    ...
    cameraPermissionMessage.value = '현재 브라우저 또는 접속 환경에서 카메라 권한 요청을 지원하지 않습니다. 실시간 표정 분석만 제외하고 면접은 계속 진행할 수 있습니다.'
  ...
  try {
    ...
    isCameraReady.value = false
    isCameraPermissionModalOpen.value = true
    cameraPermissionTitle.value = err.name === 'NotAllowedError' ? '카메라 권한이 차단되었습니다' : '카메라 연결을 확인해주세요'
    cameraPermissionMessage.value = err.name === 'NotAllowedError'
